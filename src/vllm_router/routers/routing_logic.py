...
class DisaggregatedPrefillRouter(RoutingInterface):
    ...
    def route_request(self, endpoints: List[EndpointInfo], 
        ...
        print("All endpoints: ", endpoints)
        # Find endpoints with matching model labels
        prefiller_endpoints = [e for e in endpoints if e.model_label == self.prefiller_model]
        decoder_endpoints = [e for e in endpoints if e.model_label == self.decoder_model]
        
        if not prefiller_endpoints or not decoder_endpoints:
            logger.error(f"Missing required endpoints. Prefiller: {bool(prefiller_endpoints)}, Decoder: {bool(decoder_endpoints)}")
